...
const compiledCampaign = require('../ethereum/build/Campaign.json');
 ...
 beforeEach(async () => {
    ...
    [campainaddress]=await factory.methods.getDeployedcampaigns().call();
 ...
 describe('campigns',async ()=>{
     it('delploys a factory and campaign',async ()=>{
     ...
     it('allows other to contribute',async ()=>{
     ...
     it('allows manager to make payment request',async ()=>{
         await campaign.methods.createRequest('buy some',100,accounts[1]).send({
             from:accounts[0],
             gas:'1000000'
         });
         const request=await campaign.methods.requests(0).call();
         assert('buy some',request.description);
     });

     it('aprroves request',async ()=>{
         await campaign.methods.contribute().send({
             from:accounts[0],
